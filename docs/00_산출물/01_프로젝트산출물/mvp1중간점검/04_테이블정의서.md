# 04. MVP1 테이블 정의서

## 1. 범위

본 문서는 `src/db/schema/auth.ts`와 Drizzle migration을 기준으로 MVP1 인증/검수 구현에 사용되는 PostgreSQL 테이블을 정의한다.

## 2. 공통 메타 컬럼

다음 컬럼은 `audit_log`를 제외한 주요 업무 테이블에 공통 적용된다.

| 컬럼 | 타입 | 필수 | 기본값 | 설명 |
| :--- | :--- | :---: | :--- | :--- |
| `created_sys` | varchar(50) | Y | `pdfowers` | 생성 시스템 |
| `created_at` | timestamptz | Y | `now()` | 생성 시각 |
| `created_by` | varchar(100) | Y | `system` | 생성자 |
| `updated_sys` | varchar(50) | Y | `pdfowers` | 수정 시스템 |
| `updated_at` | timestamptz | Y | `now()` | 수정 시각 |
| `updated_by` | varchar(100) | Y | `system` | 수정자 |
| `version` | integer | Y | `1` | 낙관적 잠금/버전 후보 |

## 3. ERD

```mermaid
erDiagram
    user_account ||--|| local_credential : has
    user_account ||--o{ verified_email : has
    user_account ||--o{ auth_identity : has
    user_account ||--o{ account_merge_request : request_user
    user_account ||--o{ account_merge_request : target_user
    user_account ||--o{ audit_log : actor
    common_code ||--o{ user_account : user_status
    common_code ||--o{ account_merge_request : merge_status
    common_code ||--o{ audit_log : event_type

    user_account {
      uuid user_uuid PK
      varchar display_name
      varchar primary_email
      varchar user_status_cd
      uuid merged_into_user_uuid
    }
    local_credential {
      uuid user_uuid PK,FK
      varchar login_id UK
      text password_hash
      varchar password_hash_alg_cd
    }
    verified_email {
      uuid verified_email_uuid PK
      uuid user_uuid FK
      varchar email UK
      timestamptz email_verified_at
      boolean email_notification_opt_in
    }
    auth_identity {
      uuid auth_identity_uuid PK
      uuid user_uuid FK
      varchar provider_cd
      varchar provider_user_id
      varchar email_from_provider
      timestamptz connected_at
      timestamptz last_login_at
    }
    account_merge_request {
      uuid merge_request_uuid PK
      uuid request_user_uuid FK
      uuid target_user_uuid FK
      varchar provider_cd
      varchar provider_user_id
      varchar merge_status_cd
      timestamptz expires_at
      timestamptz approved_at
      timestamptz cancelled_at
    }
    audit_log {
      uuid audit_log_uuid PK
      uuid actor_user_uuid
      varchar audit_event_type_cd
      varchar target_type_cd
      uuid target_uuid
      jsonb metadata_json
      timestamptz created_at
    }
    common_code {
      varchar code_group_cd PK
      varchar code_cd PK
      varchar code_label
      text code_description
      integer sort_order
      boolean is_active
    }
    job {
      uuid job_uuid PK
      varchar job_type_cd
      varchar job_status_cd
      jsonb payload_json
      timestamptz scheduled_at
      timestamptz started_at
      timestamptz completed_at
      text failed_reason
    }
```

## 4. `common_code`

| 컬럼 | 타입 | 키 | 필수 | 설명 |
| :--- | :--- | :---: | :---: | :--- |
| `code_group_cd` | varchar(100) | PK | Y | 코드 그룹 |
| `code_cd` | varchar(100) | PK | Y | 코드 |
| `code_label` | varchar(100) |  | Y | 표시명 |
| `code_description` | text |  | N | 설명 |
| `sort_order` | integer |  | Y | 정렬 순서 |
| `is_active` | boolean |  | Y | 활성 여부 |

주요 코드 그룹:

| 그룹 | 코드 예시 | 용도 |
| :--- | :--- | :--- |
| `USER_STATUS` | `active`, `merged` | 사용자 상태 |
| `MERGE_STATUS` | `pending`, `approved`, `cancelled`, `expired` | 계정 통합 요청 상태 |
| `AUDIT_EVENT_TYPE` | `LOGIN_SUCCESS`, `ACCOUNT_MERGE_APPROVED` 등 | 감사 이벤트 |
| `JOB_STATUS` | 후보 | 비동기 job 상태 |

## 5. `user_account`

| 컬럼 | 타입 | 키 | 필수 | 설명 |
| :--- | :--- | :---: | :---: | :--- |
| `user_uuid` | uuid | PK | Y | 내부 사용자 식별자 |
| `display_name` | varchar(100) |  | Y | 표시명 |
| `primary_email` | varchar(320) |  | Y | 대표 이메일 |
| `user_status_cd` | varchar(50) |  | Y | 사용자 상태. `active`, `merged` |
| `merged_into_user_uuid` | uuid |  | N | 통합된 경우 대표 계정 UUID |

## 6. `local_credential`

| 컬럼 | 타입 | 키 | 필수 | 설명 |
| :--- | :--- | :---: | :---: | :--- |
| `user_uuid` | uuid | PK/FK | Y | `user_account.user_uuid` |
| `login_id` | varchar(100) | UK | Y | 사용자 입력 로그인 ID |
| `password_hash` | text |  | Y | scrypt 기반 비밀번호 해시 |
| `password_hash_alg_cd` | varchar(50) |  | Y | 해시 알고리즘 코드 |

제약:

| 제약 | 내용 |
| :--- | :--- |
| `local_credential_login_id_uidx` | `login_id` 유니크 |

## 7. `verified_email`

| 컬럼 | 타입 | 키 | 필수 | 설명 |
| :--- | :--- | :---: | :---: | :--- |
| `verified_email_uuid` | uuid | PK | Y | 인증 이메일 식별자 |
| `user_uuid` | uuid | FK | Y | 사용자 UUID |
| `email` | varchar(320) | UK | Y | 이메일 주소 |
| `email_verified_at` | timestamptz |  | N | 인증 완료 시각 |
| `email_notification_opt_in` | boolean |  | Y | 이메일 알림 수신 동의 |

제약:

| 제약 | 내용 |
| :--- | :--- |
| `verified_email_email_uidx` | 이메일 전역 유니크 |

비고: 최신 MVP1 정책에서는 이메일 인증이 서비스 사용 차단 수단이 아니라 알림 수신 신뢰 수단이다. 미인증 사용자는 로그인과 서비스 이용이 가능하지만 이메일 알림 발송 대상에서는 제외한다.

## 7.1 `email_verification_request` 후보

현재 schema에는 인증번호 또는 인증 토큰 발급/만료/재발송 폐기를 추적할 별도 테이블이 없다. MVP1 이메일 인증 실제 구현 시 다음 구조를 추가 검토한다.

| 컬럼 | 타입 | 키 | 필수 | 설명 |
| :--- | :--- | :---: | :---: | :--- |
| `email_verification_request_uuid` | uuid | PK | Y | 이메일 인증 요청 식별자 |
| `user_uuid` | uuid | FK | N | 로그인 사용자 UUID. 회원가입 중이면 비어 있을 수 있음 |
| `email` | varchar(320) |  | Y | 인증 대상 이메일 |
| `purpose_cd` | varchar(50) |  | Y | `signup`, `profile`, `email_change` |
| `verification_secret_hash` | text |  | Y | 인증번호 또는 토큰 hash. 원문 저장 금지 |
| `issued_at` | timestamptz |  | Y | 발급 시각 |
| `expires_at` | timestamptz |  | Y | 만료 시각. 기본 1시간 |
| `verified_at` | timestamptz |  | N | 인증 완료 시각 |
| `revoked_at` | timestamptz |  | N | 재발송 등으로 폐기된 시각 |
| `failed_attempt_count` | integer |  | Y | 인증 확인 실패 횟수 |

제약 후보:

| 제약 | 내용 |
| :--- | :--- |
| 활성 인증 1건 | 같은 사용자 또는 같은 이메일+목적의 미완료 인증은 1건만 유효. 재발송 시 기존 건은 `revoked_at` 갱신 |
| 재발송 제한 | 사용자+이메일 기준 60초 쿨다운, 1시간 최대 5회 발송을 서비스 로직 또는 별도 rate-limit 저장소로 관리 |

## 8. `auth_identity`

| 컬럼 | 타입 | 키 | 필수 | 설명 |
| :--- | :--- | :---: | :---: | :--- |
| `auth_identity_uuid` | uuid | PK | Y | provider identity 식별자 |
| `user_uuid` | uuid | FK | Y | 연결 사용자 UUID |
| `provider_cd` | varchar(50) | UK 구성 | Y | `kakao`, `naver`, `google` |
| `provider_user_id` | varchar(255) | UK 구성 | Y | provider 사용자 식별자 |
| `email_from_provider` | varchar(320) |  | N | provider에서 받은 이메일 |
| `connected_at` | timestamptz |  | Y | 연결 시각 |
| `last_login_at` | timestamptz |  | N | 마지막 OAuth 로그인 시각 |

제약:

| 제약 | 내용 |
| :--- | :--- |
| `auth_identity_provider_user_uidx` | `(provider_cd, provider_user_id)` 유니크 |

## 9. `account_merge_request`

| 컬럼 | 타입 | 키 | 필수 | 설명 |
| :--- | :--- | :---: | :---: | :--- |
| `merge_request_uuid` | uuid | PK | Y | 통합 요청 식별자 |
| `request_user_uuid` | uuid | FK | Y | 통합 요청 계정 |
| `target_user_uuid` | uuid | FK | Y | provider identity를 보유한 대상 계정 |
| `provider_cd` | varchar(50) |  | Y | 통합 대상 provider |
| `provider_user_id` | varchar(255) |  | Y | 통합 대상 provider 사용자 식별자 |
| `merge_status_cd` | varchar(50) |  | Y | `pending`, `approved`, `cancelled`, `expired` |
| `expires_at` | timestamptz |  | Y | 만료 시각 |
| `approved_at` | timestamptz |  | N | 승인 시각 |
| `cancelled_at` | timestamptz |  | N | 취소 시각 |

UI 보완 후보:

| 항목 | 필요성 |
| :--- | :--- |
| 요청자 삭제 표시 | 요청자가 통합정보를 삭제하면 대상자 목록에서도 숨겨야 하므로 `requester_deleted_at` 또는 동등한 표시 로직 검토 |
| 대상자 삭제 표시 | 대상자 삭제는 자동 거절 처리하되 목록 표시 여부를 제어해야 하므로 `target_deleted_at` 또는 상태/감사 로그 조합 검토 |
| 거절 사유 구분 | 현재 `cancelled`가 취소/거절/삭제를 모두 표현할 수 있으므로 사용자 표시용 사유 코드 추가 여부 검토 |
| 승인 후 로그인 제한 | 승인된 대상 계정은 `user_status_cd=merged`와 `merged_into_user_uuid` 기준으로 로그인 제한 및 완료 메시지 표시 |

상태 전이:

```mermaid
stateDiagram-v2
    [*] --> pending
    pending --> approved
    pending --> cancelled
    pending --> expired
    approved --> [*]
    cancelled --> [*]
    expired --> [*]
```

## 10. `audit_log`

| 컬럼 | 타입 | 키 | 필수 | 설명 |
| :--- | :--- | :---: | :---: | :--- |
| `audit_log_uuid` | uuid | PK | Y | 감사 로그 식별자 |
| `actor_user_uuid` | uuid |  | N | 행위자 사용자 UUID |
| `audit_event_type_cd` | varchar(100) |  | Y | 감사 이벤트 코드 |
| `target_type_cd` | varchar(50) |  | N | 대상 유형 |
| `target_uuid` | uuid |  | N | 대상 식별자 |
| `metadata_json` | jsonb |  | N | 부가 메타데이터 |
| `created_at` | timestamptz |  | Y | 발생 시각 |

주요 이벤트:

| 이벤트 | 설명 |
| :--- | :--- |
| `LOCAL_USER_CREATED` | 로컬 사용자 생성 |
| `AUTH_IDENTITY_LINKED` | OAuth identity 연결 |
| `AUTH_IDENTITY_UNLINKED` | OAuth identity 해제 |
| `LOGIN_SUCCESS` | 로그인 성공 |
| `LOGIN_FAILURE` | 로그인 실패 |
| `EMAIL_VERIFICATION_REQUIRED` | 이메일 인증 필요 |
| `ACCOUNT_MERGE_REQUESTED` | 계정 통합 요청 |
| `ACCOUNT_MERGE_APPROVED` | 계정 통합 승인 |
| `ACCOUNT_MERGE_CANCELLED` | 계정 통합 취소 |
| `ACCOUNT_MERGE_EXPIRED` | 계정 통합 만료 |
| `ACCOUNT_MERGED` | 대상 계정 통합 완료 |
| `EMAIL_CHANGE_REQUESTED` | 이메일 변경 요청 |
| `EMAIL_CHANGED` | 이메일 변경 완료 |

## 11. `job`

| 컬럼 | 타입 | 키 | 필수 | 설명 |
| :--- | :--- | :---: | :---: | :--- |
| `job_uuid` | uuid | PK | Y | job 식별자 |
| `job_type_cd` | varchar(100) |  | Y | job 유형 |
| `job_status_cd` | varchar(50) |  | Y | job 상태 |
| `payload_json` | jsonb |  | Y | 실행 payload |
| `scheduled_at` | timestamptz |  | Y | 실행 예정 시각 |
| `started_at` | timestamptz |  | N | 시작 시각 |
| `completed_at` | timestamptz |  | N | 완료 시각 |
| `failed_reason` | text |  | N | 실패 사유 |

비고: MVP1 계정 통합 핵심 처리는 동기 트랜잭션 기준이며, `job`은 후속 알림/재시도/대량 이전 후보 구조다.

## 12. `system_property` 후보

현재 schema에는 시스템 설정 또는 시스템 속성 테이블이 없다. 이메일 인증 필수 여부처럼 환경별 정책으로 제어해야 하는 값을 관리하려면 다음 구조를 추가 검토한다.

| 컬럼 | 타입 | 키 | 필수 | 설명 |
| :--- | :--- | :---: | :---: | :--- |
| `property_key` | varchar(100) | PK | Y | 시스템 속성 키 |
| `property_value` | text |  | Y | 속성 값 |
| `value_type_cd` | varchar(50) |  | Y | `boolean`, `number`, `string`, `json` 등 |
| `description` | text |  | N | 속성 설명 |
| `is_active` | boolean |  | Y | 사용 여부 |

초기 후보 속성:

| 속성 키 | 타입 | 기본 후보 | 설명 |
| :--- | :--- | :--- | :--- |
| `AUTH_EMAIL_VERIFICATION_REQUIRED` | boolean | `false` | 이메일 인증 필수 여부. 필수여도 로그인은 차단하지 않고 사용자정보 화면 인증 안내로 처리 |
| `AUTH_EMAIL_VERIFICATION_EXPIRES_MINUTES` | number | `60` | 이메일 인증번호 또는 토큰 만료 시간 |
| `AUTH_EMAIL_VERIFICATION_RESEND_COOLDOWN_SECONDS` | number | `60` | 인증 메일 재발송 최소 간격 |
## 13. 테이블 및 컬럼 한글 주석 (COMMENT DDL) 관리

데이터베이스의 모든 테이블과 컬럼에는 PostgreSQL 카탈로그(`pg_description`)에 한글 설명 주석을 등록하여 관리한다.

- **DDL SQL 파일**: [`src/db/sql/table_column_comments.sql`](file:///d:/dev/workspace/git.jkoogit/PDFowers/src/db/sql/table_column_comments.sql)
- **적용 스크립트**: [`src/db/scripts/apply-comments.ts`](file:///d:/dev/workspace/git.jkoogit/PDFowers/src/db/scripts/apply-comments.ts)
- **실행 명령어**: `npm run db:comments`

```sql
-- 예시: user_account 한글 주석 DDL
COMMENT ON TABLE user_account IS '통합 사용자 계정 마스터 테이블';
COMMENT ON COLUMN user_account.user_uuid IS '사용자 고유 식별자 (UUID PK)';
COMMENT ON COLUMN user_account.display_name IS '사용자 화면 표시명 및 닉네임';
COMMENT ON COLUMN user_account.primary_email IS '사용자 대표 이메일 주소';
COMMENT ON COLUMN user_account.user_status_cd IS '사용자 계정 상태 코드 (ACTIVE, PENDING, BLOCKED, WITHDRAWN)';
```

---

## 관련 문서

- [`00_데이터관리설계기준.md`](file:///d:/dev/workspace/git.jkoogit/PDFowers/docs/03_설계관리/데이터관리/00_데이터관리설계기준.md)
- [`03_하네스_세션태스크루프_진행관리_문서DB설계.md`](file:///d:/dev/workspace/git.jkoogit/PDFowers/docs/03_설계관리/데이터관리/03_하네스_세션태스크루프_진행관리_문서DB설계.md)

---

## 작업 이력

| 작업일시 | 작업 에이전트 | 작성자 | 내용 한 줄 요약 |
| :--- | :--- | :--- | :--- |
| 2026-06-25 KST | Codex | jkoogi | 이메일 인증 요청과 시스템 속성 후보, 계정 통합 목록 표시 보완 필드 검토사항 추가 |
| 2026-09-15 KST | Gemini | jkoogi | 전체 테이블 및 컬럼 한글 주석(COMMENT DDL) 관리 표준 및 실행 스크립트 연계 추가 |

