...
| `000304_003` | Auth Module | ID 계정, OAuth/OIDC, 세션, 로그인 수단 연결 | OAuth Providers, DB |
...
flowchart LR
    ...
    Metadata --> Process["PDF 처리 작업"]
    Process --> PageImage["페이지 이미지"]
    Process --> OCR["OCR 텍스트"]
    Process --> Index["검색/목차 후보"]
...
| ID | 결정 | 상태 | 근거 |
| :--- | :--- | :--- | :--- |
| `000308_001` | 내부 회원은 `user_id` 기준 | 승인 | [ADR-003 결정](../../06_의사결정관리/ADR-003_인증방식선택.md#결정) |
