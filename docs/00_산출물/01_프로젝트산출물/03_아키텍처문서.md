...
- 내부 회원 식별자(`user_uuid`)는 사용자가 입력하는 로그인 ID와 외부 로그인 제공자 식별자와 분리한다.
...
| ID | 결정 | 상태 | 근거 |
| :--- | :--- | :--- | :--- |
| `000308_001` | 내부 회원은 `user_uuid` 기준, 사용자 입력 ID는 `login_id` 기준 | 승인 | [ADR-003 결정](../../06_의사결정관리/ADR-003_인증방식선택.md#결정) |
