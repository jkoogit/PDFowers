...
7. 마지막 리뷰 문서의 진행 내용, 미완료 항목, 후속 검토 대상, 관련 이슈/PR 번호를 새 세션 시작 요약에 반영한다.
8. 마지막 리뷰 문서에 이슈/PR 번호가 있으면 GitHub 상태를 확인해 이미 닫혔는지, 승급됐는지, 후속 작업이 남았는지 확인한다.
9. 새 세션 작업 범위를 정리한다.
10. 작업에 필요한 정책 결정사항과 구체화 필요사항을 별도로 정리한다.
11. 구현 전 결정이 필요한 항목이 없으면 작업 브랜치를 생성하고 진행한다.
...
12. 연결 이슈가 없거나, 이슈 본문/댓글에 아직 완료되지 않은 범위가 남아 있으면 닫지 말고 최종 보고에 이유를 적는다.
13. 로컬 `main`이 체크아웃 중이 아니고 `origin/main`으로 fast-forward 가능한 상태면 `git branch -f main origin/main`으로 맞춘다.
